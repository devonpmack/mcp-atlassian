...
The section ends at the next same-or-higher heading, including a heading nested in the next sibling (for example an `excerpt` or layout macro). That wrapper is left intact. Lower-level sibling headings (an `h3` after an `h2`) are still part of the section.
</Warning>
